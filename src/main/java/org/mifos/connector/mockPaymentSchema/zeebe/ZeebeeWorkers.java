...
import static org.mifos.connector.mockPaymentSchema.zeebe.ZeebeVariables.*;
...
public class ZeebeeWorkers {
    ...
    public void setupWorkers() {
            zeebeClient.newWorker()
                    ...
                    .name("mockPayerBlockFunds")
                    .maxJobsActive(workerMaxJobs)
                    .open();

            zeebeClient.newWorker()
                    .jobType("mockBookFunds")
                    .handler((client, job) -> {
                        logWorkerDetails(job);
                        if (isAmsLocalEnabled) {
                            Map<String, Object> variables = new HashMap<>();
            ...
            zeebeClient.newWorker()
                    ...
                    .name("mockReleaseBlock")
                    .maxJobsActive(workerMaxJobs)
                    .open();

            zeebeClient.newWorker()
                    .jobType("mockPayeeLookup")
                    .handler((client, job) -> {
                        logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());
                        Map<String, Object> variables = job.getVariablesAsMap();
                    ...
                    .name("mockPayeeLookup")
                    .maxJobsActive(workerMaxJobs)
                    .open();

            zeebeClient.newWorker()
                    .jobType("mockPayeeAccountStatus")
                    .handler((client, job) -> {
                        logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());
                        Map<String, Object> variables = job.getVariablesAsMap();
            ...
            zeebeClient.newWorker()
                    ...
                    .handler((client, job) -> {
                        ...
                        zeebeClient.newPublishMessageCommand()
                                .messageName("mockTransferResponse")
                                .correlationKey(variables.get(TRANSACTION_ID).toString())
